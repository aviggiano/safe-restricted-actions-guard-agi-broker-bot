...
import {
    Chains,
    LineaTokenListResponse,
    OptimismTokenListResponse,
    TokenMetadata,
    TrustWalletGithubJson,
    TrustWalletTokenMetadata,
    ViaProtocolTokenMetadata,
} from "./types.ts";
import { NATIVE_TOKENS } from "./constants.ts";
...
export class EVMTokenRegistry {
    ...
    private async fetchTokenList(
        ...
        const optimismUrl = `https://static.optimism.io/optimism.tokenlist.json`;
        const viaProtocolUrl = `https://raw.githubusercontent.com/viaprotocol/tokenlists/main/all_tokens/${chainName === 'avalanche' ? 'avax' : chainName}.json`;
        const lineaUrl = `https://api-explorer.linea.build/api/v2/tokens?type=ERC-20`;

        async function fetchOptimismTokenList(): Promise<TrustWalletTokenMetadata[]> {
            const response = await fetch(optimismUrl);
            const data: OptimismTokenListResponse = await response.json();
            return data.tokens.filter(token => token.chainId === chainId).map(token => ({
                ...token,
                type: "ERC20",
                pairs: [],
            }));
        ...
        async function fetchViaProtocolTokenList(): Promise<TrustWalletTokenMetadata[]> {
            const response = await fetch(viaProtocolUrl);
            const data: ViaProtocolTokenMetadata[] = await response.json();
        ...
        async function fetchLineaTokenList(): Promise<TrustWalletTokenMetadata[]> {
            const response = await fetch(lineaUrl);
            const data: LineaTokenListResponse = await response.json();
        ...
        const methods: Record<keyof typeof Chains, () => Promise<TrustWalletTokenMetadata[]>> = {
            linea: fetchLineaTokenList,
            optimism: fetchOptimismTokenList,
            arbitrum: fetchViaProtocolTokenList,
            avalanche: fetchViaProtocolTokenList,
            celo: fetchViaProtocolTokenList,
        };

        try {
            const tokens: TrustWalletTokenMetadata[] = await methods[chainName]()
            return tokens
        } catch (error) {
            elizaLogger.error(
